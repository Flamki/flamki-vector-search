...
if _system == "Windows":
    DATA_DIR = Path(os.getenv("LOCALAPPDATA", "")) / "Flamki Vector Search"
elif _system == "Darwin":
    DATA_DIR = Path.home() / "Library" / "Application Support" / "Flamki Vector Search"
else:
    DATA_DIR = Path(os.getenv("XDG_DATA_HOME", Path.home() / ".local" / "share")) / "Flamki Vector Search"
