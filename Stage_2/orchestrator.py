...
class Orchestrator:
	...
	def clear_skip_cache(self, name: str = None):
		...
		self.table_producers: dict[str, list[str]] = {}    # table -> task names that write it
	...
	def _build_graph(self):
		...
		# Phase 1: Map each output table to all tasks that produce it.
		self.table_producers = {}
		for task in self.tasks.values():
			for table in task.writes:
				writers = self.table_producers.setdefault(table, [])
				if writers:
					logger.debug(
						f"Shared table '{table}': written by {writers + [task.name]}"
					)
		...
		for task in self.tasks.values():
			...
			for table in task.reads:
				producer_names = self.table_producers.get(table, [])
