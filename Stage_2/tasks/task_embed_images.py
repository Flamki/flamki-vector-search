...
class EmbedImages(BaseTask):
	...
	def run(self, paths, context):
		...
		for i, (path, idx, _img) in enumerate(image_entries):
			...
			path_obj = Path(path)
			file_type = path_obj.suffix.lower().lstrip(".") or "image"
		...
		if vector_rows:
			inserted = vector_store.upsert_image_vectors(vector_rows)
			if inserted:
				logger.info(f"Mirrored {inserted} image embeddings to VectorAI backend")
