"""
Telegram bot frontend for Flamki Vector Search.
...
def run_telegram_bot(ctrl, shutdown_fn, shutdown_event: threading.Event,
    ...
    for entry in [
        ...
        CommandEntry("start", "Welcome message",
                     handler=lambda _: "Flamki Vector Search is online. Send a message to chat, or /help for commands."),
    ...
    async def _run():
        ...
        if user_id:
            ...
            try:
                await _app.bot.send_message(user_id, f"Flamki Vector Search online. {status}")
            except Exception as e:
