...
class LexicalSearch(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "file_type": {
                "type": "string",
                "description": "Filter by extension without dot (pdf, txt, mp3, etc). Use all for no filter.",
            },
            "after_date": {
                "type": "string",
    ...
    def run(self, context, **kwargs):
        ...
        sql_parts = [
            ...
            "JOIN files f ON f.path = sc.path",
        ...
        try:
            ...
            if "no such table" in str(e).lower():
                logger.info("Lexical index not built yet; returning empty lexical result set")
                return ToolResult(data=[], llm_summary=f'No lexical index results for "{query}".')
