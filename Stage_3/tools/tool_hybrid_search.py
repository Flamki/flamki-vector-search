...
from vectorai.hybrid import reciprocal_rank_fusion
...
class HybridSearch(BaseTool):
    ...
    def run(self, context, **kwargs):
        ...
        if file_type:
            lex_kwargs["file_type"] = file_type
            sem_kwargs["file_type"] = file_type
        if after_date:
            lex_kwargs["after_date"] = after_date
            sem_kwargs["after_date"] = after_date
...
def _apply_rrf(deduped_streams):
    ...
    for stream_name, docs in deduped_streams.items():
        ...
        normalized_ranked_lists.append(normalized)
    ...
    fused = reciprocal_rank_fusion(normalized_ranked_lists, k=RRF_K)
    rrf_scores = {item["path"]: item["rrf_score"] for item in fused if item.get("path")}
